...
for page_num in range(1):
    ...
    for i, recipe_list in enumerate(title):
        try:
            ...
            if temp_title != recipe_name:
                # recipe_ingredients = {}
                data = {}
                data["recipe"] = []
                ingredients = ''

            temp_title = recipe_name
            data["recipe"].append({
            ...
            for j, recipe_info in enumerate(soup.select('div[class="ingredient"]')):
                try:
                    ...
                    # print(data['recipe'])
                    for data2 in data["recipe"]:
                        data2["ingredients"].append({
            ...
            try:
                ...
                # with open('%s/%s.txt' % (path, recipe_name.replace('/','|')), 'w', encoding='utf-8') as f:
                #     f.write(recipe_text)
            except OSError as e:
...
with open('data2.json') as json_file:
    ...
    # print(data)
    for p in data['recipe']:
        print('------------------------------------------')
        print('recipe_name: ' + p['recipe_name'])
        print('recipe_url: ' + p['recipe_url'])
        print('recipe_img_url: ' + p['recipe_img_url'])
        print('ingredients:')
        for a in p['ingredients']:
            print(a['ingredient_name'] + ': ' + a['ingredient_unit'])
'''
